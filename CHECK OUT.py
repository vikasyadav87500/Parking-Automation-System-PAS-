...
for region in regionprops(label_image):
    ...
    if region_height >= min_height and region_height <= max_height and region_width >= min_width and region_width <= max_width and region_width > region_height:
        flag = 1
        plate_object.append((min_row,min_col,max_row,max_col)) 
...
def update_entry(plate):
    ...
    my_data = dict()
    my_data["vehicle no"] = plate
  
    json_data = json.dumps(my_data).encode()
    request = urllib.request.Request("https://check-in-bad3b.firebaseio.com/vehicle at exit.json", data=json_data, method="PATCH")
    ...
    try:
        ...
        print(loader.read())
        print("vehicle no at exit point" , plate)

#################################################################################################
############################################################################################################################################
if(flag == 1):
    x,y,w,h=plate_object[0]
